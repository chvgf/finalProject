...
function ShopDetail(props) {
  ...
  const handleCart = async (title, price) => {
    ...
    try {
      const result = await axios.post(
        `${process.env.REACT_APP_SERVER_DOMAIN}/shop/plusCart`,
        { title, price, postId, productCount },
  ...
  const handlePay = async () => {
    ...
    if (result.event === "done" || result.event === "issued") {
      const result = await axios.post(
        `${process.env.REACT_APP_SERVER_DOMAIN}/shop/purchaseAdd`,
        { postId, price, title, productCount },
        { withCredentials: true }
      );
      if (result.data.flag) {
    ...
    <ShopContainer>
      <div className="detail">
        ...
        <div className="detail-text">
          ...
          <span className="text2">
            <button type="button" className="countBtn" onClick={handleMinus}>
              -
            </button>
            {productCount}
            <button type="button" className="countBtn" onClick={handlePlus}>
              +
            </button>
          </span>
          <br />
          ...
          <div className="detail-btn">
            <button
              ...
              onClick={() => {
                handleCart(title, price);
              }}
            >
              장바구니
            </button>
            <button type="submit" className="buy cursor-pointer" onClick={handleBuy}>
              구매하기
            </button>
          </div>
      ...
      <TabContainer>
        <NavBox variant="tabs" defaultActiveKey="link-0" className="my-3">
          <ItemBox>
            <LinkBox eventKey="link-0" onClick={() => setShowTab("detail")}>
              상세정보
            </LinkBox>
          </ItemBox>
          <ItemBox>
            <LinkBox eventKey="link-1" onClick={() => setShowTab("review")}>
              상품리뷰
            </LinkBox>
          </ItemBox>
          <ItemBox>
            <LinkBox eventKey="link-2" onClick={() => setShowTab("qa")}>
              Q&amp;A
            </LinkBox>
          </ItemBox>
          <ItemBox>
            <LinkBox eventKey="link-3" onClick={() => setShowTab("exchange")}>
              반품/교환정보
            </LinkBox>
          </ItemBox>
        </NavBox>
        {
          {
            detail: (
              <div>
                <DetailDetail product={product} />
              </div>
            ),
            review: (
              <div>
                <DetailReview product={product} postId={postId} user={user} />
              </div>
            ),
            qa: (
              <div>
                <DetailQnA postId={postId} />
              </div>
            ),
            exchange: (
              <div>
