...
function ItemList(props) {
  ...
  useEffect(() => {
    try {
      const getList = async () => {
        const result = await axios.get(`${process.env.REACT_APP_SERVER_DOMAIN}/shop/`, { withCredentials: true });
  ...
  const handleMore = async () => {
    ...
    if (!selectedCategory) {
      const result = await axios.get(`${process.env.REACT_APP_SERVER_DOMAIN}/shop/?nextId=${productList[productList.length - 1]._id}`);
      dispatch(addMoreProducts(result.data.posts));
    } else {
      const result = await axios.get(
        `${process.env.REACT_APP_SERVER_DOMAIN}/shop/category/${selectedCategory}?nextId=${productList[productList.length - 1]._id}`
      );
      dispatch(addMoreProducts(result.data.posts));
    }
  };
  ...
  return (
    <ItemContainer>
      ...
      {moreBtn && (
        <button className="moreBtn cursor-pointer" onClick={handleMore}>
          더보기
        </button>
      )}
    </ItemContainer>
