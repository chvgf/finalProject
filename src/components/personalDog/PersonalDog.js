...
function PersonalDog(props) {
  ...
  const testDogInfo = [
    {
      ...
      character:
        '학대, 방임, 공격성을 방치한 훈육 등의 악조건을 겪지 않은 골든 리트리버는 대체로 온화한 성격을 지닌다. 강형욱 훈련사는 골든 리트리버의 온화한 성격에 대해 "100개의 옐로카드를 가졌고, 한숨 자고 나면 옐로카드가 전부 회복된다"라고 설명하기도 했다. 누군가 자신을 괴롭히더라도 99번까지는 봐줄 만큼 인내심이 좋다는 뜻.',
    },
    {
      dogType /* 허스키 */: "허스키",
      ...
      character:
        "흰색에 가까운 홍채 때문에 눈동자가 삼백안 같은 매서운 눈빛을 가지고 있으나 겉보기와 달리 허스키는 대체로 성격이 순한 품종이다.[7] 순하기 때문에 경호견으로는 적합하지 않다.",
    },
    {
      dogType /* 말티즈 */: "말티즈",
      info: "개의 한 품종이며 남유럽 지방의 섬나라인 몰타가 원산인 소형견이다. Maltese 몰타 섬에서 유래된 종이라서 실제 발음은 ‘몰티즈’인데, 한국에서는 철자를 한국식으로 읽은 ‘말티즈’로 굳어졌지만 말티즈/몰티즈가 혼재되는 편이다.",
      character:
        "활발하고 놀기를 좋아하며 주인에게 치대는 성향이 강하기 때문에 주인이 집에 들어오면 방방 뛰는 모습을 쉽게 볼 수 있다. 눈치가 빨라서 주인의 마음을 민감하게 감지할 줄 알지만 한편으로는 자기 주장을 강하게 하고 고집이 있어 밥, 간식, 산책 등 자기가 원하는 바에 대해 적극적으로 의사를 표출하는 견종이다. 더불어 자신이 원하는 바가 달성되어야 직성이 풀리는 성격이기 때문에 훈련이 잘못될 경우 욕구를 충족시키지 못한다고 판단되면 뒤끝을 보이며 휴지통을 뒤엎는 등의 문제를 일으키기 때문에 어릴 때 엄격하게 훈련을 시켜 놓지 않으면 나중에 주인이 매우 힘들어질 수 있다.",
    },
    {
      dogType /* 푸들 */: "푸들",
  ...
  const DogInfoFilter = testDogInfo.filter((DogInfoFilter) => {
    return DogInfoFilter.dogType === 로그인중?.signDogType;
  });
            ...
            {getPersonalDog?.toktokPostFilter.map((a) => {
              return (
                <div
                  className="slider"
                  onClick={() => {
                    navigate(`/community/Toktok/${a._id}`);
                  }}
                >
      ...
      <div className="ListBox">
        {DogInfoFilter.length > 0 ? (
          <div className="typeAgeInfo">
            ...
            <p>
              {로그인중?.signDogType}의 정보: {DogInfoFilter[0]?.info}
            </p>
            <p>
              {로그인중?.signDogType}의 성격: {DogInfoFilter[0]?.character}
            </p>
          </div>
        ) : (
          <div />
