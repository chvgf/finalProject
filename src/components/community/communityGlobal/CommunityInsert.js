...
function CommunityInsert(props) {
  ...
  const formDataList = async () => {
    try {
      ...
      const result = await axios.post(`${process.env.REACT_APP_SERVER_DOMAIN}/community/${insertPage}/insert`, formData, { withCredentials: true });
    ...
    <CommunityInsertWrapper>
      <label className="b">
        <label htmlFor="title" className="title-label">
          제목
          <input className="title" spellCheck="false" type="text" id="title" name="title" value={insertTitle} onChange={changeTitle} placeholder="제목 입력" />
        </label>
        ...
        <label htmlFor="imgUrl" className="imgUrl-label">
          사진 첨부
          <input className="imgUrl" type="file" accept="image/*" id="imgUrl" name="imgUrl" />
        </label>
        <button
          className="btn"
          onClick={() => {
