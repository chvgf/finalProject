  ...
  const handleSubmitValue = async () => {
    if (!title) {
      ...
      try {
        ...
        await axios.post(
          `${process.env.REACT_APP_SERVER_DOMAIN}/community/daily/insert`,
          { id, title, content, imgUrl: images, imgKey: imagesKey, author: user.signUserNicname, authorId: user._id, date },
          { withCredentials: true }
        );
        alert("게시글이 등록되었습니다.");
